...
### Usage
 ...
 - `zim.lib` — the file listing the Kiwix ZIM archives you want to monitor and update.
 - `/path/to/zim/directory/` — where Transmission puts the downloaded ZIM files.
...
Transmission's CLI commands are hard to remember and to type, so here are some
useful aliases for `~/.bash_aliases`:
...
Replace `transmission:transmission` with your own Transmission user and password.
